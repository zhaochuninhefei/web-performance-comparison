...
- 响应时间上，`web-pm-go`优势明显，以响应时间中位数来看，`web-pm-java`是`web-pm-go`的2倍以上。
- 吞吐量上，`web-pm-go`也占据明显优势，10万RPS下，比较TPS，`web-pm-go`是`web-pm-java`的大约1.8倍。

在`1000个并发用户 + 每秒100000次请求`的并发压力下，双方表现如下:
