...
## 4.2 web-pm-go的add与query
先测试`web-pm-go`的`/account/add`和`/account/query`接口。

连接池最大连接数: 100
> 由于mysql服务也部署在相同服务器上，且该服务器CPU只有4核，磁盘只有1块，因此连接池最大连接数设置到100实际上可能反而会导致性能下降。但本次测试目的是对比go和java的web框架的性能表现，数据库以及连接池方面，确保条件相同即可。

### 4.2.1 web-pm-go-account_add
接口完整URL : `http://172.17.13.44:18080/account/add`
...
### 4.2.2 web-pm-go-account_query
接口完整URL : `http://172.17.13.44:18080/account/query?id=100`
...
## 4.3 web-pm-java的add与query
先测试`web-pm-java`的`/account/add`和`/account/query`接口。

连接池最大连接数: 100
> 由于mysql服务也部署在相同服务器上，且该服务器CPU只有4核，磁盘只有1块，因此连接池最大连接数设置到100实际上可能反而会导致性能下降。但本次测试目的是对比go和java的web框架的性能表现，数据库以及连接池方面，确保条件相同即可。


### 4.3.1 web-pm-java-account_query
接口完整URL : `http://172.17.13.44:18081/account/query?id=101`
...
### 4.3.2 web-pm-java-account_add
接口完整URL : `http://172.17.13.44:18081/account/add`
