...
对`go+gin+gorm`与`java+springboot+mybatis`进行相同条件下的性能测试，比较双方性能表现。
...
> 1. 由于服务器硬件与数据库是同一套环境，磁盘IO的影响是一样的，不会对性能比较造成影响，所以测试数据没有列出。
> 
> 2. 内存方面由于数据量并不大，都没有超过1G，对性能测试过程以及测试结论没有影响，所以测试数据没有列出。

## 1.1 单表读写比较
在`500个用户并发 + 每秒5000次请求`的并发压力下，双方的单表读写的性能表现如下所示：

| 项目 | 接口 | web服务CPU使用率 | 数据库CPU使用率 | 响应时间中位数 | 吞吐量 |
| --- | --- | --- | --- | --- | --- |
| web-pm-go | /account/add 插入数据 | 100% | 150% | 85ms | 1939 |
| web-pm-java | /account/add 插入数据 | 150% | 120% | 182ms | 1698 |
| web-pm-go | /account/query 主键查询 | 130% | 150% | 44ms | 4128 |
| web-pm-java | /account/query 主键查询 | 260% | 70% | 137ms | 2785 |

> web服务与数据库部署在同一个4核服务器上，web服务的CPU使用率过高时，会影响到数据库的CPU使用。

## 1.2 简单API比较
在`1000个用户并发 + 每秒10000次请求`的并发压力下，双方表现如下:
...
从结果上看，**高并发场景下的简单HttpAPI，在服务器硬件相同的条件下，相比java+springboot，go+gin使用更少的资源消耗能够提供更好的性能表现，特别是响应时间。**
...
- 响应时间上，`web-pm-go`优势明显，以响应时间中位数来看，`web-pm-java`是`web-pm-go`的2到3倍，并发越高，`web-pm-go`优势越明显。
- 吞吐量上，`web-pm-go`占据微弱优势。
...
2. 在5000RPS的性能压力下，`web-pm-go-account_add`的吞吐量为`1900+`，响应时间中位数为`85ms`。
3. 在140秒内，`web-pm-go-account_add`共新增数据`271408`条。
...
2. 在5000RPS的性能压力下，`web-pm-go-account_query`的吞吐量为`4128+`，响应时间中位数为`44ms`。这个吞吐量并未比简单API的吞吐量下降太多，主要是由于查询SQL利用了MySQL缓存，极大缓解了IO阻塞。
...
#### 分析
1. 容器的CPU使用率相比`web-pm-go`要多`50%`，而MySQL的CPU使用率反而下降了`30%`，两者之和依然距离`400%`有一段距离。推测是`web-pm-java`在高并发时吞吐量的下降导致打给MySQL的数据变少，进而导致MySQL并不需要满载运转，因而其CPU使用率下降。
2. 在5000RPS的性能压力下，`web-pm-java-account_add`的吞吐量为`1698+`，响应时间中位数为`182ms`。相比`web-pm-go`，吞吐量有所下降，响应时间明显更久。
3. 在140秒内，`web-pm-java-account_add`共新增数据`239747`条。比`web-pm-go`少了约3万件。
...
#### 分析
1. 容器的CPU使用率高达`260%`，可以看到明显影响到了MySQL的CPU使用率，使其下降到`70%`。此时MySQL明显没有满负荷运行，这应该会明显影响到吞吐量。查询SQL重复，可以一直使用MySQL缓存，导致性能压力从数据库读取IO阻塞向`web-pm-java`容器服务转移，于是容器的CPU使用率非常高，以至于抢夺了MySQL的CPU使用，使得数据库读取的吞吐量明显下降。
2. 在5000RPS的性能压力下，`web-pm-java-account_query`的吞吐量为`2784+`，响应时间中位数为`137ms`。相比`web-pm-go`，不管是吞吐量还是响应时间都明显不如。
