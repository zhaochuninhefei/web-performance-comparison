...
4. 删除表`accounts`，并重建该表，参考`db/drop_create_table.sql`
5. 执行`jmeter/web-pm-java-account_add/web-pm-java-account_add.jmx`
6. 执行`jmeter/web-pm-java-account_query/web-pm-java-account_query.jmx`

## 4.2 web-pm-go的add与query
先测试`web-pm-go`的`/account/add`和`/account/query`接口。

### web-pm-go-account_add
